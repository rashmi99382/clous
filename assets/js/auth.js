import {
  ...
  signIn,
  signUp
} from "https://esm.sh/aws-amplify@6.15.6/auth";
...
document.getElementById("verify-form")?.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    setMessage("verify-message", error.message ?? "Verification failed.", "error");
  }
});
...
document.getElementById("login-form")?.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    setMessage("login-message", error.message ?? "Login failed.", "error");
  }
});
...
document.getElementById("forgot-form")?.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    setMessage("forgot-message", error.message ?? "Could not send reset code.", "error");
  }
});
...
document.getElementById("reset-form")?.addEventListener("submit", async (event) => {
  ...
  try {
    ...
    setMessage("reset-message", error.message ?? "Password reset failed.", "error");
  }
});
